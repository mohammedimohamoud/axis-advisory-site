...
function applyLanguage(lang) {
  ...
  // Update select option elements (service dropdown)
...
/* =============================================================
   9. CONTACT FORM — ASYNC SUBMISSION TO /api/contact
   ============================================================= */
function initContactForm() {
  ...
  /** Show or clear a per-field inline error */
  ...
  form.addEventListener('submit', async e => {
    ...
    if (!nameEl?.value.trim()) {
      setFieldError('err-name', 'fg-name', t.form_err_required);
      hasError = true;
    }
    if (!companyEl?.value.trim()) {
      setFieldError('err-company', 'fg-company', t.form_err_required);
      hasError = true;
    }
    if (!emailEl?.value.trim() || !emailPat.test(emailEl.value.trim())) {
    ...
    try {
      ...
      // Server returned a specific validation error
      if (data.error === 'required_fields') {
        if (formErrEl) {
          formErrEl.textContent = t.form_err_required;
        ...
        if (formErrEl) {
          formErrEl.textContent = t.form_err_server;
          formErrEl.hidden = false;
        }
      }
    } catch {
